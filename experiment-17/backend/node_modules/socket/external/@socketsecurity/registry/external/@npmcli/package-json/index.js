...
let hosts_1$1
let hasRequiredHosts$1
function requireHosts$1() {
  if (hasRequiredHosts$1) {
    return hosts_1$1
  }
...
let parseUrl$1
let hasRequiredParseUrl$1
function requireParseUrl$1() {
  if (hasRequiredParseUrl$1) {
    return parseUrl$1
  }
...
let fromUrl$1
let hasRequiredFromUrl$1
function requireFromUrl$1() {
  if (hasRequiredFromUrl$1) {
    return fromUrl$1
  }
...
function requireNormalizeData() {
  ...
  const hostedGitInfo = requireLib$9()
...
function requireSpawn() {
  ...
  hasRequiredSpawn = 1
  const spawn = requireLib$7()
  const promiseRetry = requirePromiseRetry()
  const { log } = requireLib$a()
      ...
      if (typeof pkg.bin === 'object') {
        for (const binKey in pkg.bin) {
          ...
          if (binTarget !== pkg.bin[binKey]) {
            changes?.push(
              `"bin[${base}]" script name ${binTarget} was invalid and removed`
            )
          }
  ...
  async function asyncSteps(pkg, { steps, root, changes }) {
    ...
    if (steps.includes('binDir') && data.directories?.bin && !data.bin) {
      ...
      const bins = await lazyLoadGlob()('**', {
        cwd: path.resolve(pkg.path, binPath)
      })
...
function requireLib() {
  ...
  const { normalize, syncNormalize } = requireNormalize()
  ...
  class PackageJson {
    ...
    syncNormalize(opts = {}) {
      opts.steps = this.constructor.normalizeSteps.filter(
        s => s !== '_attributes'
      )
