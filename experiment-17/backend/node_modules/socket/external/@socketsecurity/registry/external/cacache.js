...
const require$$0$4 = require('fs/promises')
...
function requireWithTempDir() {
  ...
  const { join, sep } = require$$0$2
  const getOptions = requireGetOptions()
  const { mkdir, mkdtemp, rm } = require$$0$4
...
function requireMoveFile() {
  ...
  const { dirname, join, resolve, relative, isAbsolute } = require$$0$2
  const fs = require$$0$4
...
function requireEntryIndex() {
  ...
  const crypto = require$$0$1
  const { appendFile, mkdir, readFile, readdir, rm, writeFile } = require$$0$4
  const { Minipass } = requireCommonjs$4()
  const path = require$$0$2
  const ssri = requireLib$5()
...
function requireVerify() {
  ...
  hasRequiredVerify = 1
  const { mkdir, readFile, rm, stat, truncate, writeFile } = require$$0$4
  const contentPath = requirePath()
  const fsm = requireLib$1()
  const glob = requireGlob()
